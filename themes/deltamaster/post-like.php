 ...
 * (4) Front end button
 */
function getPostLikeLink( $blog_id, $post_id ) {
	switch_to_blog($blog_id);
	...
	if ( AlreadyLiked( $post_id ) ) {
		...
		$title = esc_attr( 'Already Liked' );
		$heart = '<i id="icon-like" class="icon-like"></i>';
		$output = '<a href="#" class="likes'.$class.'" data-post_id="'.$post_id.'" data-blog_id="'.$blog_id.'" title="'.$title.'" disabled="disabled">'.$heart.'&nbsp;'.$count.'</a>';
	
	} else {
		$class = esc_attr( '' );
		$title = esc_attr( 'Like' );
		$heart = '<i id="icon-unlike" class="icon-unlike"></i>';
		$output = '<a href="#" class="jm-post-like likes'.$class.'" data-post_id="'.$post_id.'" data-blog_id="'.$blog_id.'" title="'.$title.'">'.$heart.'&nbsp;'.$count.'</a>';
