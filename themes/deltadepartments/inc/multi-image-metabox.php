...
function add_image_metabox(){
	$cpts = apply_filters('images_cpt', array('gallery'));
	foreach($cpts as $cpt)
		add_meta_box('imagesliees', __('Add Banner'), "imagesliees", $cpt, 'normal', 'core');
}
...
function imagesliees($post){
	...
	wp_enqueue_script( 'img-mb', get_bloginfo('template_url').('/inc/get-images.js'), array( 'jquery','media-upload','thickbox','set-post-thumbnail' ) );
