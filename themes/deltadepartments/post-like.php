...
function like_scripts() {
	wp_enqueue_script( 'jm_like_post', theme_url.'/js/post-like.min.js');
...
function getPostLikeLink( $post_id ) {
	...
	if ( AlreadyLiked( $post_id ) ) {
		...
		$output = '<a href="#" class="likes'.$class.'" data-post_id="'.$post_id.'" title="'.$title.'" disabled="disabled">'.$heart.'&nbsp;'.$count.'</a>';
	
	} else {
		$class = esc_attr( '' );
		$title = esc_attr( 'Like' );
		$heart = '<i id="icon-unlike" class="icon-unlike"></i>';
		$output = '<a href="#" class="jm-post-like likes'.$class.'" data-post_id="'.$post_id.'" title="'.$title.'">'.$heart.'&nbsp;'.$count.'</a>';
